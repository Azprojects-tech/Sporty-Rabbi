...
test('corners are wired into the daily desk and recorded separately', () => {
  const server = read('../src/server.js');
  const view = read('../../frontend/src/components/DailyDesk.jsx');
  assert.match(server, /cornersDesk.recordPredictions/);
  assert.match(server, /predictCorners: m => cornersDesk.predict/);
  assert.match(view, /c.corners.lines/);
});
