...
function recordText(name, record, previous) {
  ...
  if (p && w != null && d != null) {
    ...
    text += ' Comparison is with last season overall, not matched rounds.';
    if (played < 6) text += ' This season is still a small sample.';
...
function teamChanges(name, id, info, seasonStart, asOf, transferStart = seasonStart) {
  ...
  } else parts.push(`${name}: current coach/coach change unverified.`);
  if (transferStart != null && Array.isArray(info?.transfers)) {
    ...
    parts.push('Transfer records do not prove lineup availability or improved team strength.');
...
export function buildGroundedAnalystNote(analysis = {}, match = {}, evidence = {}) {
  ...
  if (['LIVE','1H','2H','HT','ET'].includes(match.status)) sections.push({ label: 'Evidence at selection',
    text: `Checked at ${match.matchMinutes ?? '?'}', score ${match.score || 'unavailable'}. Home–away: shots on target ${pair(match.shots)}, xG ${pair(match.xg)}. Accumulated match totals, not the last five minutes.` });
  ...
  if (start != null && end != null && end > start && asOf >= start && asOf <= end) {
    ...
    sections.push({ label: 'Season stage', text: `${fraction < 0.25 ? 'Beginning' : fraction < 0.75 ? 'Middle' : 'Late stage'} of the recorded ${match.season} season (${season.start} to ${season.end}). Based on the calendar, not rounds played.` });
  } else sections.push({ label: 'Season stage', text: 'Verified season dates unavailable; the month alone does not identify the season stage.' });
  for (const side of ['home', 'away']) {
    ...
    sections.push({ label: `${name}: late goals`, text: timing?.sampled
      ? `From 80 minutes onward (including stoppage time), scored in ${timing.scored} and conceded in ${timing.conceded} of ${timing.sampled} verified recent league games.${timing.sampled < timing.requested ? ` Event coverage is partial (${timing.sampled}/${timing.requested}); unknown games are excluded.` : ''}${timing.sampled < 5 ? ' Fewer than five verified games: too small to call a reliable pattern.' : ''} This is history, not a prediction of another goal.`
  ...
  sections.push({ label: 'What this means now', text: gf?.active
    ? `${gf.summary}. Goal Fest is a trajectory signal, not a calibrated probability. Historical/team context does not change the model weights in this release.`
    : `${gf?.summary || 'No verified Goal Fest signal is currently available.'} Late goals, coach changes or transfers alone do not establish a betting edge.` });
